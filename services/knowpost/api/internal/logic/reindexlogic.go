...
func NewReindexLogic(ctx context.Context, svcCtx *svc.ServiceContext) *ReindexLogic {
	return &ReindexLogic{Logger: logx.WithContext(ctx), ctx: ctx, svcCtx: svcCtx}
}
func (l *ReindexLogic) WithId(id int64) *ReindexLogic { l.id = id; return l }

func (l *ReindexLogic) Reindex() (*types.Empty, error) {
	uid, ok := ctxdata.GetUserId(l.ctx)
	if !ok {
		return nil, errorx.New(errorx.CodeUnauthorized, "missing user id")
	}
	if _, err := l.svcCtx.KnowPostRpc.Reindex(l.ctx, &pb.ReindexReq{Id: l.id, CreatorId: uid}); err != nil {
		return nil, err
	}
